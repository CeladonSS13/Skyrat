...
/datum/preferences/ui_static_data(mob/user)
	...
	for(var/_provider in SSttsSilero.tts_providers)
		var/datum/tts_provider/provider = SSttsSilero.tts_providers[_provider]
		providers += list(list(
			"name" = provider.name,
	...
	var/list/seeds = list()
	for(var/_seed in SSttsSilero.tts_seeds)
		var/datum/tts_seed/seed = SSttsSilero.tts_seeds[_seed]
		seeds += list(list(
			"name" = seed.name,
			"value" = seed.value,
...
/datum/preferences/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch (action)
		if("listen")
			...
			if((phrase in TTS_PHRASES) && (seed_name in SSttsSilero.tts_seeds))
