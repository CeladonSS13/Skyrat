 ...
 */
#define MODULAR_SAVEFILE_VERSION_MAX_CELADON 1

#define MODULAR_SAVEFILE_UP_TO_DATE_CELADON -1

#define VERSION_TRANSLATE_RELIGION 1

/**
 * Checks if the modular side of the savefile is up to date.
 * If the return value is higher than 0, update_character_celadon() will be called later.
 */
/datum/preferences/proc/savefile_needs_update_celadon(list/save_data)
	var/savefile_version = save_data["modular_version_1984"] // legacy string, don't change

	if(save_data.len && savefile_version < MODULAR_SAVEFILE_VERSION_MAX_CELADON)
		return savefile_version

	return MODULAR_SAVEFILE_UP_TO_DATE_CELADON


/// Loads the modular customizations of a character from the savefile
/datum/preferences/proc/load_character_celadon(list/save_data)
	if(!save_data)
		save_data = list()

	var/needs_update = savefile_needs_update_celadon(save_data)
	if(needs_update >= 0)
		update_character_celadon(needs_update, save_data) // needs_update == savefile_version if we need an update (positive integer)

/// Brings a savefile up to date with modular preferences. Called if savefile_needs_update_celadon() returned a value higher than 0
/datum/preferences/proc/update_character_celadon(current_version, list/save_data)
	if(current_version < VERSION_TRANSLATE_RELIGION)
		update_religion_celadon(save_data)


/datum/preferences/proc/update_religion_celadon(list/save_data)
	...
	) : alist("ERROR" = "RELIGION NAMES GOT CHANGED AND HAVE FEWER NAMES! UPDATE /datum/preferences/proc/update_religion_celadon(list/save_data)")
...
/datum/preferences/proc/save_character_celadon(list/save_data)
	save_data["modular_version_1984"] = MODULAR_SAVEFILE_VERSION_MAX_CELADON // legacy string, don't change

#undef MODULAR_SAVEFILE_VERSION_MAX_CELADON
#undef MODULAR_SAVEFILE_UP_TO_DATE_CELADON
