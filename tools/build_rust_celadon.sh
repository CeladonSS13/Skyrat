...
if command -v rsync >/dev/null 2>&1; then
    rsync -a --mkpath "$SRC_DIR/target/i686-unknown-linux-gnu/release/librust_celadon.so" "$PROJECT_ROOT/"
else
    cp -f "$SRC_DIR/RUST_REMOTE/target/i686-unknown-linux-gnu/release/librust_celadon.so" "$PROJECT_ROOT/librust_celadon.so"
fi

if [ ! $? -eq 0 ]; then
  echo "Failed to copy librust_celadon.so"
  exit 107
fi

cd $PROJECT_ROOT
chmod +x librust_celadon.so
