...
if /i "%buildLocal%"=="false" (
    xcopy "%~dp0\..\rust_cached\rust_celadon.dll" "%~dp0\.." /Y /q
    IF %ERRORLEVEL% NEQ 0 (
        ECHO Failed to copy rust_celadon.dll. Is file exist: rust_cached\rust_celadon.dll?
        exit 117
    )
    ECHO Copied rust_cached\rust_celadon.dll
...
xcopy "%~dp0\..\RUST\target\i686-pc-windows-msvc\release\rust_celadon.dll" "%~dp0\.." /Y /q
IF %ERRORLEVEL% NEQ 0 (
    ECHO Failed to copy rust_celadon.dll
    exit 107
)
