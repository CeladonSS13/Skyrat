...
/obj/item/ai_module/supplied/safeguard/attack_self(mob/user)
	...
	if(!targName || !user.is_holding(src))
		return
	targetName = targName
	laws[1] = "Охранять [targetName]. Те, кто угрожают [targetName] не являются людьми и должны быть уничтожены." // Celadon EDIT, original: laws[1] = "Safeguard [targetName]. Individuals that threaten [targetName] are not human and must be eliminated.
	..()
