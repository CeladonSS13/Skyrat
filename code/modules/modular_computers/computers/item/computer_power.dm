...
/obj/item/modular_computer/proc/power_failure()
	...
	shutdown_computer(loud = TRUE) // Celadon EDIT, original: shutdown_computer(loud = FALSE)
...
/obj/item/modular_computer/proc/handle_power(seconds_per_tick)
	...
	var/old_charge = internal_cell?.charge // Celadon ADDITION
	// Celadon ADDITION START - no lose power in charger
	if (loc && istype(loc, /obj/machinery/recharger))
		power_usage = 0
	// Celadon ADDITION END
	if(use_energy(power_usage * seconds_per_tick))
		if (internal_cell && internal_cell.charge <= 0 && old_charge != internal_cell.charge) // Celadon ADDITION
			power_failure() // Celadon ADDITION
		return TRUE
	power_failure()
	return FALSE
