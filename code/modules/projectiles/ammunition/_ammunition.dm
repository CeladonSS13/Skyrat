...
/obj/item/ammo_casing/proc/add_notes_ammo()
	...
	if(!ispath(exam_proj) || pellets == 0)
		return
	var/datum/weakref/was_chambered_at_shared = was_chambered_at // Celadon ADDITION
	...
	// No dividing by 0
	if(initial_damage)
		readout += "Most monkeys our legal team subjected to these [span_warning(caliber)] rounds succumbed to their wounds after [span_warning("[HITS_TO_CRIT((initial(exam_proj.damage) * proj_damage_mult) * pellets)] shot\s[was_chambered_desc][dmg_desc_normal_text]")] at point-blank, taking [span_warning("[pellets] shot\s")] per round." // Celadon EDIT, original:	readout += "Most monkeys our legal team subjected to these [span_warning(caliber)] rounds succumbed to their wounds after [span_warning("[HITS_TO_CRIT((initial(exam_proj.damage) * proj_damage_mult) * pellets)] shot\s")] at point-blank, taking [span_warning("[pellets] shot\s")] per round."
	if(initial_stamina)
		readout += "[!readout.len ? "Most monkeys" : "More fortunate monkeys"] collapsed from exhaustion after [span_warning("[HITS_TO_CRIT((initial(exam_proj.stamina) * proj_damage_mult) * pellets)] impact\s[was_chambered_desc][dmg_desc_normal_stamina_text]")] of these [span_warning("[caliber]")] rounds." // Celadon EDIT, original: readout += "[!readout.len ? "Most monkeys" : "More fortunate monkeys"] collapsed from exhaustion after [span_warning("[HITS_TO_CRIT((initial(exam_proj.stamina) * proj_damage_mult) * pellets)] impact\s")] of these [span_warning("[caliber]")] rounds."
