...
/obj/vehicle/sealed/mecha/vehicle_move(direction, forcerotate = FALSE)
	if(!COOLDOWN_FINISHED(src, cooldown_vehicle_move))
		return FALSE
	COOLDOWN_START(src, cooldown_vehicle_move, get_movedelay()) // Celadon EDIT, original: COOLDOWN_START(src, cooldown_vehicle_move, movedelay)
	...
	set_glide_size(DELAY_TO_GLIDE_SIZE(get_movedelay())) // Celadon EDIT, original: set_glide_size(DELAY_TO_GLIDE_SIZE(movedelay))
