...
/obj/vehicle/ridden/golfcart/proc/fake_glide(direct)
	...
	child.pixel_x += px
	child.pixel_y += py
	var/calculated_movedelay = get_movedelay() // Celadon ADDITION
	animate(src, calculated_movedelay, pixel_x=0, pixel_y=0) // Celadon EDIT, original: animate(src, movedelay, pixel_x=0, pixel_y=0)
	animate(child, calculated_movedelay, pixel_x=0, pixel_y=0) // Celadon EDIT, original: animate(child, movedelay, pixel_x=0, pixel_y=0)
	for (var/mob/buckled in (buckled_mobs + child.buckled_mobs))
		...
		buckled.pixel_x += px
		buckled.pixel_y += py
		animate(buckled, calculated_movedelay, pixel_x=inital_pixel_x, pixel_y=inital_pixel_y) // Celadon EDIT, original: animate(buckled, movedelay, pixel_x=inital_pixel_x, pixel_y=inital_pixel_y)
		if (buckled.client)
			...
			client.pixel_x += px
			client.pixel_y += py
			animate(client, calculated_movedelay, pixel_x=initial_client_pixel_x, pixel_y=initial_client_pixel_y) // Celadon EDIT, original: animate(client, movedelay, pixel_x=initial_client_pixel_x, pixel_y=initial_client_pixel_y)
