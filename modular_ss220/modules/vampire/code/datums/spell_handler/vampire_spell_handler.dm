...
/datum/spell_handler/vampire/can_cast(mob/user, charge_check, show_message, datum/action/cooldown/spell/spell)
	...
	vampire.nullified) && !fullpower) // above 100 nullification vampire powers are useless
		if(show_message)
	...
	//chapel check
	var/owner_area = get_area(user)
	if(is_type_in_typecache(owner_area, GLOB.holy_areas) && !fullpower)
		if(show_message)
...
/datum/spell_handler/vampire/spend_spell_cost(mob/user, spell)
...
/datum/spell_handler/vampire/after_cast(list/targets, mob/user, spell)
	if(!required_blood)
		return
	var/datum/antagonist/vampire/vampire = user?.mind?.has_antag_datum(/datum/antagonist/vampire)
	if(!vampire)
