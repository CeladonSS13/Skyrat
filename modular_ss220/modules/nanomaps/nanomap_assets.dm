/datum/asset/simple/nanomaps
	assets = list(
		"Delta Station_nanomap_z1.png" = 'icons/_nanomaps/Delta Station_nanomap_z1.png',
		"Ice Box Station_nanomap_z1.png" = 'icons/_nanomaps/Ice Box Station_nanomap_z1.png',
		"Ice Box Station_nanomap_z2.png" = 'icons/_nanomaps/Ice Box Station_nanomap_z2.png',
		"Ice Box Station_nanomap_z3.png" = 'icons/_nanomaps/Ice Box Station_nanomap_z3.png',
		"MetaStation_nanomap_z1.png" = 'icons/_nanomaps/MetaStation_nanomap_z1.png',
		"Tramstation_nanomap_z1.png" = 'icons/_nanomaps/Tramstation_nanomap_z1.png',
		"Tramstation_nanomap_z2.png" = 'icons/_nanomaps/Tramstation_nanomap_z2.png',
		"Lavaland_nanomap_z1.png" = 'icons/_nanomaps/Lavaland_nanomap_z1.png',
		// NOVA maps
		"Catwalk Station_nanomap_z1.png" = 'icons/_nanomaps/Catwalk Station_nanomap_z1.png',
		"Catwalk Station_nanomap_z2.png" = 'icons/_nanomaps/Catwalk Station_nanomap_z2.png',
		"Snowglobe Station_nanomap_z1.png" = 'icons/_nanomaps/Snowglobe Station_nanomap_z1.png',
		"Snowglobe Station_nanomap_z2.png" = 'icons/_nanomaps/Snowglobe Station_nanomap_z2.png',
		"Snowglobe Station_nanomap_z3.png" = 'icons/_nanomaps/Snowglobe Station_nanomap_z3.png',
		"Void Raptor_nanomap_z1.png" = 'icons/_nanomaps/Void Raptor_nanomap_z1.png',
		"Wawastation_nanomap_z1.png" = 'icons/_nanomaps/Wawastation_nanomap_z1.png',
		"Wawastation_nanomap_z2.png" = 'icons/_nanomaps/Wawastation_nanomap_z2.png',
	)
